...
public class CargoManifest {
	...
	private Stack<FreightContainer> stack;
	...
	public CargoManifest(Integer numStacks, Integer maxHeight, Integer maxWeight)
			throws ManifestException {
		// If any parameters are negative, throw ManifestException
		if (numStacks < 0 | maxHeight < 0 | maxWeight < 0) {
			throw new ManifestException("Values cannot be negative");
		}
		// Assign parameters to variables
		this.numStacks = numStacks;
		this.maxHeight = maxHeight;
		this.maxWeight = maxWeight;
		// Initialise the manifest as an ArrayList
		manifest = new ArrayList<Stack<FreightContainer>>();
		// Add stacks to the Manifest
	...
	public void loadContainer(FreightContainer newContainer)
			throws ManifestException {
		// If adding container would exceed ship's weight max, throw
		// ManifestException
		if (getActualWeight() + newContainer.getGrossWeight() > maxWeight) {
			throw new ManifestException(
					"Container too heavy. Would exceed ship's weight limit.");
		}
		// If container is already in manifest, throw ManifestException
		if (whichStack(newContainer.getCode()) != null) {
			throw new ManifestException(
					"Container Code already in Cargo Manifest");
		}
		// Create a hashmap with a summary of every stack. Key = stack index.
		// Value = Its Height / Its Class type
		...
		for (int i = 0; i < stackSpaceAndType.size(); i++) {
			...
			// Split stacks's HashMap value into an array. Index 0 is its
			// height, index 1 is its class
			String[] stackStringHeight = classAndHeightString.split("/");
			// Check if newContainer is the same type of the actual stack and
			// also if the there's an available space in the stack
			if (stackStringHeight[1].equals(newContainer.getClass().toString())
					&& Integer.parseInt(stackStringHeight[0]) < this.maxHeight) {
				// If so controller gets 1, means it was possible to find a
			...
			// Else checks if stack is empty, if so controller gets 1 (form the
			// same reason that before) and adds container and breaks the loop
		...
		// If it was not possible to find a place to this container (controller
		// == 0) then throws an exception
		if (controller == 0)
			throw new ManifestException(
					"No suitable space can be found for this container");
	}
		...
		for (int i = 0; i < manifest.size(); i++) {
			for (int j = 0; j < manifest.get(i).size(); j++)
				actualWeight = actualWeight
						+ manifest.get(i).elementAt(j).getGrossWeight();
		}
	...
	private HashMap<Integer, String> checkStacks() {
		...
		// Iterates through all the stacks checking if its empty or what is its
		// height/class type
		for (int i = 0; i < manifest.size(); i++)
			if (manifest.get(i).size() > 0)
				stackSpaceAndType.put(i, manifest.get(i).size() + "/"
						+ manifest.get(i).peek().getClass().toString());
			else
	...
	public void unloadContainer(ContainerCode containerId)
			throws ManifestException {
		int controller = 0;
		// Iterates through all stacks checking if the container at the top
		// (peek) has the same code of the one the user is trying to unload.
		for (int i = 0; i < manifest.size(); i++) {
			if (!manifest.get(i).isEmpty()) {
				if (manifest.get(i).peek().getCode().equals(containerId)) {
					// If exist a container on the top of the stack with the
					// code, then remove it and assign 1 to the controller
					manifest.get(i).pop();
					controller = 1;
				}
			}
		...
		// If it was not possible to find a container at the top of any stack
		// (controller == 0) then throws an exception
		if (controller == 0)
			throw new ManifestException(
					"Container Code not on the top of any container stack or is not on board");
	...
	public Integer whichStack(ContainerCode queryContainer) {
		// Initialise variable at null meaning container not found.
		Integer whichStk = null;
		// For all stacks in the manifest, if they contain the queryContainer,
		// record the stack number.
		for (int i = 0; i < manifest.size(); i++) {
			if (manifest.get(i).contains(queryContainer)) {
				whichStk = i + 1;
			}
		}
		// Return whatever value has been recorded for 'whichStk'
		return whichStk;
	}
	...
	public Integer howHigh(ContainerCode queryContainer) {
		// If the container is not on board, return null
		if (manifest.get(whichStack(queryContainer)).indexOf(queryContainer) == -1) {
			return null;
			// Otherwise, return the height of the container in the stack
		} else {
			return (manifest.get(whichStack(queryContainer)).indexOf(
					queryContainer) + 1);
		}
	}
	...
	public FreightContainer[] toArray(Integer stackNo) throws ManifestException {
		// If exists a stack with the given index then convert it to an array
		// and returns it
	...
	public String toString(ContainerCode toFind) {
		...
		for (int i = 0; i < manifest.size(); ++i) {
			...
			try {
				currentStack = new ArrayList<FreightContainer>(
						Arrays.asList(this.toArray(i)));
